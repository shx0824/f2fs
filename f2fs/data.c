...
{
	//return mpage_readpage(page, get_data_block_ro); /*deleted by Flyhigh*/
	/* added by Flyhigh */
	int ret;
					...
					struct writeback_control *wbc)
{
	...
	const pgoff_t end_index = ((unsigned long long) i_size)
							>> PAGE_CACHE_SHIFT;
	//unsigned offset;
	unsigned offset = 0; /*motified by Flyhigh*/
...
write:
	...
	//	err = do_write_data_page(page); /*deleted by Flyhigh*/
...
#endif
	/*ended by shixing*/
	//	err = f2fs_readpage(sbi, page, dn.data_blkaddr, READ_SYNC); /*deleted by shixing*/
		if (err)
			return err;
		lock_page(page);
		if (!PageUptodate(page)) {
